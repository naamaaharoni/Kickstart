...
/***/ function(module, exports, __webpack_require__) {
	...
	var __WEBPACK_AMD_DEFINE_ARRAY__, __WEBPACK_AMD_DEFINE_RESULT__;!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(/*! privates/core */ 4), __webpack_require__(/*! Base */ 22), __webpack_require__(/*! Billing */ 13), __webpack_require__(/*! privates/utils */ 9), __webpack_require__(/*! Activities */ 17), __webpack_require__(/*! Settings */ 20),
	        ...
	        __webpack_require__(/*! WindowPlacement */ 24), __webpack_require__(/*! Worker */ 27), __webpack_require__(/*! PubSub */ 28), __webpack_require__(/*! Preview */ 30), __webpack_require__(/*! Dashboard */ 31), __webpack_require__(/*! Theme */ 23), __webpack_require__(/*! Counters */ 32), __webpack_require__(/*! Features */ 33), __webpack_require__(/*! privates/urlUtils */ 8), __webpack_require__(/*! Data */ 29), __webpack_require__(/*! Services */ 34)], __WEBPACK_AMD_DEFINE_RESULT__ = function (core, Base, Billing, utils, Activities, Settings,
	              Contacts, Utils, Styles, Events, Error, Media, WindowOrigin,
	              WindowPlacement, Worker, PubSub, Preview, Dashboard, Theme, Counters, Features, urlUtils, Data, Services) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(/*! privates/utils */ 9), __webpack_require__(/*! privates/reporter */ 10), __webpack_require__(/*! privates/postMessage */ 7), __webpack_require__(/*! privates/responseHandlers */ 14)], __WEBPACK_AMD_DEFINE_RESULT__ = function (utils, reporter, postMessage, responseHandlers) {
	    ...
	    function isValidServiceType(serviceType) {
	        return Object.keys(SERVICE_TYPES).some(function (key) {
	            return SERVICE_TYPES[key] === serviceType;
	        });
	    }

	    var open = function open(target, success, config, failure) {
	        if (!utils.isObject(target)) {
	            reporter.reportSdkError('Mandatory argument - target, must be an object');
	            return;
	        }
	        if (!isValidServiceType(target.serviceType)) {
	            reporter.reportSdkError('Invalid argument - target.serviceType, must be one of Wix.Services.Types');
	            return;
	        }
	        ...
	        var callback = function callback(response) {
	            responseHandlers.handleDataResponse(response, success, failure);
	        };
	        postMessage.sendMessage(postMessage.MessageTypes.OPEN_APP, args, callback);
	    };

	    var getConfig = function getConfig(success, failure) {
	        if (!utils.isFunction(success)) {
	            reporter.reportSdkError('Mandatory argument - success, must be a function');
	            return;
	        }
	        ...
	        postMessage.sendMessage(postMessage.MessageTypes.GET_PROVIDER_CONFIG, undefined, callback);
	    };

	    return {
	        /**
	         * This constant map all of the service types that are supported.
	         * @enum
	         * @memberof Wix.Services
	         * @since 1.65.0
	         * @example
	         *
	         * Wix.Services.Types.EMAIL
	         */
	        Types: SERVICE_TYPES,
	        /**
	         * Sub module for Wix.Services that holds the client functions
	         * @object
	         * @memberof Wix.Services
	         * @since 1.65.0
	         * @example
	        ...
	        Client: {
	            /**
	             * Either opens a provider directly, or lets the user choose and then open a provider. The behavior is based on the value of target.
	             *
	             * If the user specifies a target.providerId and a target.serviceType, the service is invoked directly.
	             * Meaning, the method opens the service provider app without showing the modal first.
	             *
	             * If target.providerId is not specified or target.providerId does not exist (or some other error):
	             * Open a window showing a list of providers installed on the site for the requested service type.
	             *
	             * @function
	             * @memberof Wix.Services.Client
	             * @since 1.65.0
	             * @param {Object} [target] An Object containing service and/or providers information.
	             * @param {Function} [success] Called when the Provider calls done. May include data, depending on the service provider.
	             * @param {Object} [config] Configuration to be sent to the front end service.
	             * @param {Function} [failure] Callback that receives an error if:
	             *                                                                  1. The serviceType is unknown
	             *                                                                  2. The providerId is unknown
	             *                                                                  3. The serviceType and the providerId do not match
	             *                                                                  4. Provider returned an error
	             *                                                                  5. Provider cancelled
	             * @example
	             *
	             * Wix.Services.Client.open(target, success, [config], [failure])
	             */
	            open: open
	        },
	        /**
	         * Sub module for Wix.Services that holds the provider functions
	         * @object
	         * @memberof Wix.Services
	         * @since 1.65.0
	         * @example
	         *
	         * Wix.Services.Provider
	         */
	        Provider: {
	            /**
	             * Used by: Service provider app
	             * Gets the configuration object, if sent by the service client.
	             *
	             * @function
	             * @memberof Wix.Services.Provider
	             * @since 1.65.0
	             * @param {Function} [success] Receives the object sent by the service client, if one was sent.
	             * @param {Function} [failure] Callback that receives an error
	             * @example
	             *
	             * Wix.Services.Provider.getConfig (success, failure)
	             */
	            getConfig: getConfig
	        }
	    };
