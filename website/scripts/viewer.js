...
//const {importSync} = require('./importScriptAsAmdModule');
const {importScriptsAsNpmModule} = __webpack_require__(1);

const initAppForPage = async () => {
    const scriptUrl = buildUrl();
    self._module = importScriptsAsNpmModule(self, scriptUrl);
};

const buildUrl = () => {
    const elementorySupport = self.elementorySupport;
    const params = elementorySupport.queryParameters;
    const suggestedValue = Date.now().toString();

    return `${elementorySupport.baseUrl}/appstudio.viewer.js?${params}&module-name=bundle&cacheKiller=${suggestedValue}`;
};
  
function createControllers() {
    return self._module.createControllers(...arguments)
}
...
function importScriptsAsNpmModule(workerGlobalScope, url, scriptScope = workerGlobalScope) {
  ...
  try {
    ...
    console.error(`Failed to import script:, url: ${url}`) //eslint-disable-line no-console
  }
